...
Source0:	http://www.squidguard.org/Downloads/%{name}-%{version}.tar.gz
...
%configure2_5x \
    --with-ldap \
    --with-sg-config=%{_sysconfdir}/squid/%{name}.conf \
    --with-sg-logdir=/var/log/squidGuard \
    --with-sg-dbhome=%{_datadir}/%{name}-%{version}/db
...
Q=%{buildroot}%{_datadir}/%{name}-%{version}
...
install -d %{buildroot}%{_datadir}/%{name}-%{version}/contrib
install -d %{buildroot}%{_datadir}/%{name}-%{version}/db/{advertising,bannedsource,banneddestination}
install -d %{buildroot}%{_datadir}/%{name}-%{version}/db/{timerestriction,lansource,privilegedsource}
install -d %{buildroot}%{_datadir}/%{name}-%{version}/db/{porn,adult,audio-video,forums,hacking,redirector}
install -d %{buildroot}%{_datadir}/%{name}-%{version}/db/{warez,ads,aggressive,drugs,gambling,publicite,violence}
...
# log & error files
touch %{buildroot}/var/log/%{name}/%{name}.{log,error}
touch %{buildroot}/var/log/%{name}/advertising.log

# conf file
install %{SOURCE1} %{buildroot}/etc/squid/%{name}.conf.sample
cp -af %{SOURCE3} %{SOURCE4} %{buildroot}/var/www/cgi-bin
rm -rf %{buildroot}%{_datadir}/%{name}-%{version}/samples/dest
...
mv %{_datadir}/%{name}-%{version} %{_datadir}/%{name}-%{version}.bk

%triggerpostun -- squidGuard <= 1.2.0-13mdv2007.1
mv %{_datadir}/%{name}-%{version}.bk %{_datadir}/%{name}-%{version}
